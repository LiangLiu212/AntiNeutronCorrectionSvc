...
void AntiNeutronTrk::setJpsirunNo(const int runNo){
		...
		if(runNo > 0){
				efficiency_flg = -1;
				i_runNo = -runNo;
			//	cout << "AntiNeutronTrk::setJpsirunNo(const int runNo), ERROR! It is design for MC!" << endl;
			//	abort();
		}
		if(i_runNo <= -9947 && i_runNo >=-10878){
				f1 = f_0912_1;
				f2 = f_0912_2;
				hnbar_ratio  = hnbar_ratio0912;
				hnbar_ratio_err = hnbar_ratio0912_err;
		}
		if(i_runNo <= -27255 && i_runNo >=-28236){
				f1 = f_0912_1;
				f2 = f_0912_2;
				hnbar_ratio  = hnbar_ratio0912;
				hnbar_ratio_err = hnbar_ratio0912_err;
		}
		if(i_runNo <= -52940 && i_runNo >=-54976){
				f1 = f_18_1;
				f2 = f_18_2;
				hnbar_ratio  = hnbar_ratio18;
				hnbar_ratio_err = hnbar_ratio18_err;
		}
		if(i_runNo <= -55861 && i_runNo >=-56546){
				f1 = f_18_1;
				f2 = f_18_2;
				hnbar_ratio  = hnbar_ratio18;
				hnbar_ratio_err = hnbar_ratio18_err;
		}
		if(i_runNo <= -56788 && i_runNo >=-59015){
				f1 = f_18_1;
				f2 = f_18_2;
				hnbar_ratio  = hnbar_ratio18;
				hnbar_ratio_err = hnbar_ratio18_err;
		...
		if(NULL == hnbar_ratio){
				cout << "Cannot read TH1 ratio!" << endl;
				abort();
		}
		if(NULL == hnbar_ratio_err){
				cout << "Cannot read TH1 ratio!" << endl;
				abort();
		}
...
void AntiNeutronTrk::setErrorMatrix(AntiNeutronTrk *nbarTrk){
		if(energy_flg ==  -1){
				cout << "Cannot call AntiNeutronTrk::setErrorMatrix after AntiNeutronTrk::setEnergy" << endl;
				abort();
		}
...
//+++++++++++++++++++++++++++++ sampling a new error matrix for Antineutron Correction. ++++++++++++++++++++++++++++++
		double costhe = cos(nbarTrk->theta());
		double nbar_Energy = nbarTrk->energy();
		if(nbar_Energy < 0.48) nbar_Energy = 0.48;
		...
		double rtn_nbardelphi = gnbar_delphi->Eval(rdm_nbardphi->Rndm());
		nbar_posi.dtheta = rtn_nbardelthe;
		nbar_posi.dphi = rtn_nbardelphi;
		delete gnbar_delthe;
		delete gnbar_delphi;
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		...
		if(fabs(cos(nthe)) > 0.8){  		// a naive way to weight the efficiency when the cos theta out of the detection acceptance................
				if(pmom_ratio_idx == 1){
						double x[2],  y[2];
						x[0] = 0.04*pmom_ratio_idx -0.02;
						x[1] = 0.04*pmom_ratio_idx +0.02;
						y[0] = hnbar_ratio->GetBinContent(pmom_ratio_idx, costhe_ratio_idx);
						y[1] = hnbar_ratio->GetBinContent(pmom_ratio_idx+1, costhe_ratio_idx);
						eff_interpolation = new TGraph(2, x, y);
						ratio = eff_interpolation->Eval(pmom_val);
				}
				else if(pmom_ratio_idx == 50){
						double x[2], y[2];
						x[0] = 0.04*pmom_ratio_idx -0.06;
						x[1] = 0.04*pmom_ratio_idx -0.02;
						y[0] = hnbar_ratio->GetBinContent(pmom_ratio_idx-1, costhe_ratio_idx);
						y[1] = hnbar_ratio->GetBinContent(pmom_ratio_idx, costhe_ratio_idx);
						eff_interpolation = new TGraph(2, x, y);
						ratio = eff_interpolation->Eval(pmom_val);
				}
				else if( pmom_ratio_idx > 1 && pmom_ratio_idx < 50){
						double x[3], y[3];
						x[0] = 0.04*pmom_ratio_idx -0.06;
						x[1] = 0.04*pmom_ratio_idx -0.02;
						x[1] = 0.04*pmom_ratio_idx +0.02;
						y[0] = hnbar_ratio->GetBinContent(pmom_ratio_idx-1, costhe_ratio_idx);
						y[1] = hnbar_ratio->GetBinContent(pmom_ratio_idx, costhe_ratio_idx);
						y[2] = hnbar_ratio->GetBinContent(pmom_ratio_idx+1, costhe_ratio_idx);
						eff_interpolation = new TGraph(3, x, y);
						ratio = eff_interpolation->Eval(pmom_val);
				}
		//		ratio = 0.9*ratio;
		}
		...
		if( ratran <= ratio) {
				...
				double rtn_nbarhits = gnbar_hits->Eval(rdm_nbarhits->Rndm(), 0);
				...
				int E_idx2 = floor((rtn_nbarenergy - 0.4)/0.1) + 1;
				int E_idx;
...
//#ifdef MOMENTUM_INTERPOLATION

				TGraph *gnbar2_angle0;
				TGraph *gnbar2_angle1;
				TGraph *gnbar0_angle0;
				TGraph *gnbar0_angle1;
				TGraph *momE_interpolation;
				TGraph *momB_interpolation;
				double rdm_angle = rdm_nbarangle->Rndm();
				
				if(p_idx == 1){
						gnbar0_angle0 = new TGraph();
						gnbar0_angle1 = new TGraph();
						p2validx = "_p" + Itoa(p_idx+1, 10);
						TString nbar2_angle0name = "nbar_angle0" + p2validx + theidx + Evalidx;				// angle C.D.F of barrel
						gnbar2_angle0 = (TGraph*)f1->Get(nbar2_angle0name);
						if(NULL == gnbar2_angle0) {
								cout << "cannot open file : angle0" << endl;
								cout << "cannot open file : angle0  " << p2validx  << " " << theidx << " " << Evalidx <<   endl;
								abort();
						}
						TString nbar2_angle1name = "nbar_angle1" + p2validx + theidx + Evalidx;				// angle C.D.F of barrel
						gnbar2_angle1 = (TGraph*)f1->Get(nbar2_angle1name);
						if(NULL == gnbar2_angle1) {
								cout << "cannot open file : angle1" << endl;
								abort();
						}
						double x[2], y[2];
						x[0] = 0.1*p_idx - 0.05;
						x[1] = 0.1*p_idx + 0.05;
						y[0] = gnbar_angle0->Eval(rdm_angle);
						y[1] = gnbar2_angle0->Eval(rdm_angle);
						momE_interpolation = new TGraph(2, x, y);
						y[0] = gnbar_angle1->Eval(rdm_angle);
						y[1] = gnbar2_angle1->Eval(rdm_angle);
						momB_interpolation = new TGraph(2, x, y);
				}

				else if(p_idx > 1 && p_idx < 12){
						p0validx = "_p" + Itoa(p_idx-1, 10);
						p2validx = "_p" + Itoa(p_idx+1, 10);
						TString nbar2_angle0name = "nbar_angle0" + p2validx + theidx + Evalidx;				// angle C.D.F of barrel
						gnbar2_angle0 = (TGraph*)f1->Get(nbar2_angle0name);
						if(NULL == gnbar2_angle0) {
						...
						TString nbar0_angle0name = "nbar_angle0" + p0validx + theidx + Evalidx;				// angle C.D.F of barrel
						gnbar0_angle0 = (TGraph*)f1->Get(nbar0_angle0name);
						if(NULL == gnbar0_angle0) {
								cout << "cannot open file : angle0" << endl;
								cout << "cannot open file : angle0  " << p0validx  << " " << theidx << " " << Evalidx <<   endl;
								abort();
						}
						TString nbar2_angle1name = "nbar_angle1" + p2validx + theidx + Evalidx;				// angle C.D.F of barrel
						gnbar2_angle1 = (TGraph*)f1->Get(nbar2_angle1name);
						...
						TString nbar0_angle1name = "nbar_angle1" + p0validx + theidx + Evalidx;				// angle C.D.F of barrel
						gnbar0_angle1 = (TGraph*)f1->Get(nbar0_angle1name);
						if(NULL == gnbar0_angle1) {
								cout << "cannot open file : angle1" << endl;
								abort();
						}
						...
						x[1] = 0.1*p_idx - 0.05;
						x[2] = 0.1*p_idx + 0.05;
						y[0] = gnbar0_angle0->Eval(rdm_angle);
						y[1] = gnbar_angle0->Eval(rdm_angle);
						y[2] = gnbar2_angle0->Eval(rdm_angle);
						momE_interpolation = new TGraph(3, x, y);
						y[0] = gnbar0_angle1->Eval(rdm_angle);
						y[1] = gnbar_angle1->Eval(rdm_angle);
						y[2] = gnbar2_angle1->Eval(rdm_angle);
						momB_interpolation = new TGraph(3, x, y);

				}
				else if(p_idx == 12){
						gnbar2_angle0 = new TGraph();
						gnbar2_angle1 = new TGraph();
						p0validx = "_p" + Itoa(p_idx-1, 10);
						TString nbar0_angle0name = "nbar_angle0" + p0validx + theidx + Evalidx;				// angle C.D.F of barrel
						gnbar0_angle0 = (TGraph*)f1->Get(nbar0_angle0name);
						if(NULL == gnbar0_angle0) {
				...
				double ang_S = 1.0 + 0.05*(pmom_val - 0.6)/0.6;   // considering the binning effection.......................

				if(theta_ratio > rdm_theta_ratio){
						rtn_nbarangle = momE_interpolation->Eval(pmom_val);
						do{
						...
						rtn_nbarangle = momB_interpolation->Eval(pmom_val);
						do{
				...
				if(cos(rtn_nbartheta) > 0.826){
						nbar_posi.z =140.0;
						nbar_posi.x = 140.0*tan(rtn_nbartheta)*cos(rtn_nbarphi);
						nbar_posi.y = 140.0*tan(rtn_nbartheta)*sin(rtn_nbarphi);
						nbar_posi.dx = 140.0*tan(rtn_nbartheta + rtn_nbardtheta) * rtn_nbardphi *sin(nbar_posi.phi);
						nbar_posi.dy = 140.0*tan(rtn_nbartheta + rtn_nbardtheta) * rtn_nbardphi *cos(nbar_posi.phi);
						nbar_posi.dz = 140.0*rtn_nbardtheta/(cos(rtn_nbartheta)*cos(rtn_nbartheta));
				}
				else if(cos(rtn_nbartheta) < -0.826){
						nbar_posi.z = -140.0;
						nbar_posi.x = -140.0*tan(rtn_nbartheta)*cos(rtn_nbarphi);
						nbar_posi.y = -140.0*tan(rtn_nbartheta)*sin(rtn_nbarphi);
						nbar_posi.dx = -140.0*tan(rtn_nbartheta + rtn_nbardtheta) * rtn_nbardphi *sin(nbar_posi.phi);
						...
						nbar_posi.z = 95.5 / tan(rtn_nbartheta);
						nbar_posi.x = 95.5* cos(rtn_nbarphi);
						nbar_posi.y = 95.5 * sin(nbar_posi.phi);
						nbar_posi.dx = 95.5 * rtn_nbardphi *sin(nbar_posi.phi);
						nbar_posi.dy = 95.5 * rtn_nbardphi *cos(nbar_posi.phi);
						nbar_posi.dz = 95.5*rtn_nbardtheta/(sin(rtn_nbartheta)*sin(rtn_nbartheta));
				}
				...
				delete gnbar2_angle0; gnbar2_angle0=NULL;
				delete gnbar2_angle1; gnbar2_angle1=NULL;
				delete gnbar0_angle0; gnbar0_angle0=NULL;
				delete gnbar0_angle1; gnbar0_angle1=NULL;
				delete momE_interpolation; momE_interpolation=NULL;
				delete momB_interpolation; momB_interpolation=NULL;
